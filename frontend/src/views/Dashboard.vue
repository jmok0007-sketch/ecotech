...
// Cancers with published links to Pb / Hg / Cd exposure (IARC + reviews)
...
// ---------------------------------------------------------------------------
// Big-number computeds (these tell the story)
// ---------------------------------------------------------------------------

// Total air emissions: latest year vs. earliest year
...
// Linked-cancer cases: 1982 vs 2010 (the dataset endpoints)
...
// Per-cancer growth %  (1982 → 2010, sex='persons')
const perCancerGrowth = computed(() => {
  const result = []
  for (const c of HEAVY_METAL_LINKED) {
    const points = healthData.value.filter(
      (r) => r.sex === 'persons' && r.cancer_type === c.name,
    )
    if (!points.length) {
...
// Top 3 polluting states (cumulative)
...
// Metal mix (which metal dominates)
...
// ---------------------------------------------------------------------------
// Chart 1 — Pollution + Cancer side-by-side single chart (dual axis)
// One picture instead of two. Both lines, normalized.
// ---------------------------------------------------------------------------
const trendChartRef = ref(null)
let trendChart = null

const trendChartOption = computed(() => {
  // Normalize both series to 100 at their own first year so they overlay visually
...
// ---------------------------------------------------------------------------
// Chart 2 — Top polluting states (horizontal bar, easier to read names)
// ---------------------------------------------------------------------------
...
const stateChartOption = computed(() => {
  ...
  return {
    ...
    xAxis: {
      ...
      axisLabel: {
        color: '#4b5563',
        formatter: (v) => (v >= 1_000_000 ? `${(v / 1_000_000).toFixed(1)}M kg` : `${(v / 1000).toFixed(0)}k`),
      },
      splitLine: { lineStyle: { color: '#f3f4f6' } },
    },
    yAxis: {
...
<template>
  <div class="dashboard">
    <!-- ============================================================== -->
    <!-- HERO — single question, plain answer                            -->
    <!-- ============================================================== -->
    <section class="hero">
      ...
      <p class="lede">
        ...
        <strong>lead, mercury and cadmium</strong> inside them leak into the air,
        water, and soil. These metals are linked to cancer of the lung, kidney,
        bladder, prostate, and more. Here is what Australia's data is showing us.
      </p>
    </section>
    ...
    <template v-else>
      <!-- ============================================================ -->
      <!-- THE BIG TAKEAWAY — one sentence + one chart                   -->
      <!-- ============================================================ -->
      <section class="takeaway">
        <p class="eyebrow centered">The big picture</p>
        <h2>Pollution and cancer are rising together.</h2>
        <p class="takeaway-lede">
          Both heavy metal pollution from industry <em>and</em> cancers linked to
          those metals have grown sharply in Australia over the last few decades.
          One chart, two lines, same direction.
        </p>
        ...
        <div class="big-stats">
          <div class="big-stat danger">
            ...
            <p class="big-hint">
              Compared to {{ earliestYear }} (latest year: {{ latestYear }})
            </p>
          </div>
        ...
        <div class="trend-chart-wrap">
          <p class="chart-help">
            Both series are indexed to <strong>100</strong> at their first year so
            you can compare growth side by side.
          </p>
      ...
      <!-- ============================================================ -->
      <!-- THE 10 CANCERS — card grid (replaces the old line chart)      -->
      <!-- ============================================================ -->
      <section class="card-section">
        <h2>10 cancers tied to heavy-metal exposure</h2>
        <p class="section-lede">
          These cancers all have published links to lead, mercury, or cadmium
          exposure. Here's how each one has grown in Australia from 1982 to 2010.
        </p>

        <div class="cancer-grid">
          <article
            v-for="c in perCancerGrowth"
            :key="c.name"
            class="cancer-card"
            :class="{ 'cancer-up': (c.change || 0) > 0, 'cancer-flat': Math.abs(c.change || 0) < 5 }"
          >
            <div class="cancer-top">
              <span class="cancer-icon">{{ c.icon }}</span>
              <span class="cancer-metal">Linked to {{ c.metal }}</span>
            </div>
            <h3>{{ c.name }}</h3>
            <p class="cancer-change">{{ pct(c.change) }}</p>
            <p class="cancer-detail">
              {{ formatNumber(c.latest) }} cases in 2010
            </p>
          </article>
        </div>
      </section>
      ...
      <!-- ============================================================ -->
      <!-- WHAT'S IN THE AIR — the metal mix, simple stacked bar         -->
      <!-- ============================================================ -->
      <section class="card-section">
        <h2>What's in the air? Three metals, three risks.</h2>
        <p class="section-lede">
          Australia's industrial pollution data tracks three heavy metals known to
          come from electronics, batteries, and old screens.
        </p>
        ...
        <div class="metal-mix">
          <div
            ...
            class="metal-card"
          >
            <div class="metal-bar-track">
              <div
                class="metal-bar-fill"
                :class="`metal-${m.metal.toLowerCase()}`"
                :style="{ width: `${m.pct}%` }"
              />
            </div>
            <div class="metal-text">
              <h3>{{ m.metal }}</h3>
              <p class="metal-share">{{ m.pct.toFixed(1) }}% of all heavy-metal air emissions</p>
      ...
      <section class="card-section">
        ...
        <p class="section-lede">
          ...
          of Australia's reported heavy-metal air pollution. Local action — including
          responsible e-waste recycling — has the biggest impact in these areas.
        </p>
      ...
      <section class="meaning-card">
        ...
        <p class="meaning-foot">
          When you drop an old device at a verified e-waste centre, those grams
          stay out of the air, water, and food chain — for generations.
        </p>
      </section>
      ...
      <section class="method">
        ...
        <ul>
          <li><strong>What we can show:</strong> pollution and metal-linked cancers have both risen over the same decades.</li>
          <li><strong>What we can't show:</strong> our cancer dataset has no state breakdown, so we can't link emissions to specific local cases.</li>
          <li><strong>Sources:</strong> AIHW Cancer Data (1982–2010) · National Pollutant Inventory (1998–2024) · IARC monographs.</li>
        </ul>
      </section>
      ...
      <section class="cta">
        ...
        <router-link to="/disposal-locations" class="cta-btn">
...
.dashboard {
  ...
  color: #173a29;
  background:
    radial-gradient(circle at 95% 0%, rgba(129, 199, 132, 0.10), transparent 22%),
...
.state.error { background: #fef2f2; border-color: #fecaca; color: #991b1b; }
...
.big-icon { font-size: 36px; display: block; margin-bottom: 8px; }
.big-label {
  font-size: 13px;
  font-weight: 600;
  color: #4b5563;
...
.big-stat.danger .big-value { color: #dc2626; }
.big-stat.warning .big-value { color: #7c3aed; }
.big-hint { font-size: 12px; color: #6b7280; margin: 0; }
...
.cancer-up .cancer-change { color: #dc2626; }
.cancer-flat .cancer-change { color: #6b7280; }
.cancer-detail {
  font-size: 12px;
  color: #6b7280;
  margin: 0;
...
.metal-lead    { background: linear-gradient(90deg, #ef4444, #dc2626); }
.metal-mercury { background: linear-gradient(90deg, #a78bfa, #7c3aed); }
.metal-cadmium { background: linear-gradient(90deg, #38bdf8, #0891b2); }
.metal-text h3 {
  margin: 0 0 4px;
  font-size: 17px;
  color: #1f2937;
...
.cta h2 { font-size: 26px; margin: 0 0 6px; }
.cta p { font-size: 15px; margin: 0; opacity: 0.95; }
.cta-btn {
  background: #fff;
  color: #15803d;
  padding: 14px 24px;
...
.cta-btn:hover { transform: translateY(-1px); box-shadow: 0 6px 14px rgba(0,0,0,0.18); }

/* =============================================================== */
/* RESPONSIVE                                                      */
/* =============================================================== */
@media (max-width: 720px) {
  .hero, .takeaway, .card-section, .meaning-card { padding: 24px; border-radius: 18px; }
  .big-value { font-size: 42px; }
  .metal-card { grid-template-columns: 1fr; gap: 12px; }
  .cancer-card h3 { min-height: 0; }
  .trend-chart, .state-chart { height: 280px; }
  .cta { padding: 24px; }
